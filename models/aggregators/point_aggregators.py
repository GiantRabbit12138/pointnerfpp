...
from ..backbone.fieldnet import PointConvs 
from ..backbone.acne import Acne
...
class PointAggregator(torch.nn.Module):
    ...
    def __init__(self, opt):
        ...
        self.viewdir_channels = (2 * self.num_viewdir_freqs * 3 + self.opt.view_ori * 3) if self.num_viewdir_freqs > 0 else self.opt.view_ori
    ...
    def acne_init(self, opt, block_init_lst):
        ...
        in_channels = dist_xyz_dim + in_channels + self.dist_dim 
        in_channels = in_channels + (3 if "1" in list(opt.point_color_mode) else 0) + (
            4 if "1" in list(opt.point_dir_mode) else 0)

        for idx_layer in range(self.opt.num_point_emb_level+1):
            if self.opt.num_point_emb_level > 0:
                block_name_append = f"_{idx_layer}"
            else:
                block_name_append = ""

            block_name = 'block' + block_name_append
            out_channels = opt.shading_feature_num
            block = Acne(in_channels, out_channels, opt)
    ...
    def viewmlp_init(self, opt, block_init_lst):
        ...
        in_channels_ori = in_channels

        # if self.opt.agg_opt == "pointconv":
        #     opt.shading_feature_num = opt.shading_feature_num // 2
        self.is_reduce_size_list = []
        assert self.opt.agg_opt in ['pts_planes', 'pointnet', 'planes']
        for idx_layer in range(self.opt.num_point_emb_level+1):
            is_reduce_size = idx_layer < self.opt.num_point_emb_level and self.opt.agg_opt in ["pts_planes", "planes"]
            is_reduce_size = False  # let's now disable it. 
            self.is_reduce_size_list += [is_reduce_size]
            if self.opt.agg_opt == "pointconv":
                in_channels = opt.point_features_dim 
                in_channels += (2 * opt.num_feat_freqs * in_channels if opt.num_feat_freqs > 0 else 0)  # remove xyz coordinate embedding.
            else: 
                in_channels = in_channels_ori
            
            if is_reduce_size:
                in_channels = opt.point_features_dim
                in_channels += dist_xyz_dim 

            add_pts_dim = 0
            if self.opt.agg_opt == "pointnet":
                add_pts_dim = (self.opt.num_point_emb_level - idx_layer) * opt.point_features_dim
                add_pts_dim = add_pts_dim +  (2 * opt.num_feat_freqs * add_pts_dim if opt.num_feat_freqs > 0 else 0)
            in_channels += add_pts_dim

            if self.opt.num_point_emb_level <= 0 and self.opt.backbone_opt == "none":
                block_name_append = "" 
            else:
                block_name_append = f"_{idx_layer}"

            block_name = 'block1' + block_name_append
            if opt.shading_feature_mlp_layer1 > 0:
                # out_channels = opt.shading_feature_num if not is_reduce_size else opt.shading_feature_num // 2
                out_channels = opt.shading_feature_num
                block1 = []
                for i in range(opt.shading_feature_mlp_layer1 if not is_reduce_size else 1):
                    block1.append(nn.Linear(in_channels, out_channels))
                    block1.append(self.act(inplace=True))
                    in_channels = out_channels
                block1 = nn.Sequential(*block1)
                setattr(self, block_name, block1)
                block_init_lst.append(getattr(self, block_name))
            else:
                setattr(self, block_name, self.passfunc)

            block_name = 'block2' + block_name_append
            if opt.shading_feature_mlp_layer2 > 0:
                ...
                out_channels = opt.shading_feature_num if not is_reduce_size else opt.shading_feature_num // 2
                block2 = []
                for i in range(opt.shading_feature_mlp_layer2 if not is_reduce_size else 1):
                    block2.append(nn.Linear(in_channels, out_channels))
                    block2.append(self.act(inplace=True))
                    in_channels = out_channels
                block2 = nn.Sequential(*block2)
                setattr(self, block_name, block2)
                block_init_lst.append(getattr(self, block_name))
            else:
                setattr(self, block_name, self.passfunc)


            block_name = 'block3' + block_name_append
            if opt.shading_feature_mlp_layer3 > 0:
                in_channels = in_channels + (3 if "1" in list(opt.point_color_mode) else 0) + (
                    4 if "1" in list(opt.point_dir_mode) else 0)
                out_channels = opt.shading_feature_num
                block3 = []
                for i in range(opt.shading_feature_mlp_layer3 if not is_reduce_size else 1):
                    block3.append(nn.Linear(in_channels, out_channels))
                    block3.append(self.act(inplace=True))
                    in_channels = out_channels
                block3 = nn.Sequential(*block3)
                setattr(self, block_name, block3)
                block_init_lst.append(getattr(self, block_name))
            else:
                setattr(self, block_name, self.passfunc)

            block_name = 'block_conv' + block_name_append
            if self.opt.agg_opt == "pointconv":
                # PointConvs(loc_enc_len, num_weights, feat_dim, self.output_feat, cfg)
                block4 = PointConvs(dist_xyz_dim, 8, opt.shading_feature_num, opt.shading_feature_num, opt)
                setattr(self, block_name, block4)
    ...
    def acne(self, sampled_color, sampled_Rw2c, sampled_dir, sampled_conf, sampled_embedding, sampled_xyz_pers, sampled_xyz, sample_pnt_mask, sample_loc, sample_loc_w, sample_ray_dirs, vsize, weight, pnt_mask_flat, pts, viewdirs, total_len, ray_valid, in_shape, dists):
        ...
        if self.opt.agg_intrp_order == 0:
            ...
            if self.opt.dist_xyz_freq != 0:
                ...
                dists_flat_freq = positional_encoding(dists_flat, self.opt.dist_xyz_freq)
                dists_flat = torch.cat([dists_flat, dists_flat_freq], dim=-1)
        ...
        device = sampled_embedding.device
        segment_idx_full = torch.arange(
            B*R*SR, dtype=torch.long, device=device, 
            )[:, None].expand(-1, K).reshape((-1))
        segment_idx = segment_idx_full[pnt_mask_flat]
        _, reverse_idx = segment_idx.unique(return_inverse=True) # re-indexing. 
        sparse_weight_prior = weight.view(-1)[pnt_mask_flat]
        feat, sparse_weight_out= self.block(feat, [reverse_idx, sparse_weight_prior])
        weight_holder = torch.zeros_like(weight.view(-1))
        weight_holder[pnt_mask_flat] = sparse_weight_out.squeeze(-1) 
    ...
    def viewmlp(self, sampled_color, sampled_Rw2c, sampled_dir, sampled_conf, sampled_embedding, sampled_xyz_pers, sampled_xyz, sample_pnt_mask, sample_loc, sample_loc_w, sample_ray_dirs, vsize, weight, pnt_mask_flat, pts, viewdirs, total_len, ray_valid, in_shape, dists):
        ...
        if self.opt.agg_opt == "pointconv":
            feat = self.block_conv(dists_flat, feat)
    ...
    def forward(self, sampled_color, sampled_Rw2c, sampled_dir, sampled_conf, sampled_embedding, sampled_xyz_pers, sampled_xyz, sample_pnt_mask, sample_loc, sample_loc_w, sample_ray_dirs, vsize, grid_vox_sz, idx_layer=0):
        ...
        assert self.opt.mid_conf_mode in ["learned_normalized", "learned", "constant_ones"]
        if self.opt.mid_conf_mode == "learned_normalized":         
            agg_weight = weight * conf_coefficient
            agg_weight = agg_weight / torch.clamp(torch.sum(agg_weight, dim=-1, keepdim=True), min=1e-8) 
        else:
            agg_weight = weight * conf_coefficient 

        if self.opt.num_point_emb_level <= 0 and self.opt.backbone_opt == "none":
            output, _ = getattr(self, self.which_agg_model, None)(sampled_color, sampled_Rw2c, sampled_dir, sampled_conf, sampled_embedding, sampled_xyz_pers, sampled_xyz, sample_pnt_mask, sample_loc, sample_loc_w, sample_ray_dirs, vsize, agg_weight, pnt_mask_flat, pts, viewdirs, total_len, ray_valid, in_shape, dists)
            ...
            output, viewdirs = getattr(self, self.which_agg_model+"_multilevel", None)(
                sampled_color, sampled_Rw2c, sampled_dir, sampled_conf, sampled_embedding, sampled_xyz_pers, sampled_xyz, 
                sample_pnt_mask, sample_loc, sample_loc_w, sample_ray_dirs, vsize, agg_weight, pnt_mask_flat, pts, 
                viewdirs, total_len, ray_valid, in_shape, dists, idx_layer=idx_layer)

            if (self.opt.sparse_loss_weight <=0) and ("conf_coefficient" not in self.opt.zero_one_loss_items) and self.opt.prob == 0:
                weight, conf_coefficient = None, None 

            return output.view(in_shape[:-1] + (output.shape[-1],)), viewdirs.view(in_shape[:-1] + (viewdirs.shape[-1],)), ray_valid.view(in_shape[:-1]), weight, conf_coefficient
        
    
    def get_radiance(self, feat, viewdirs):
        alpha_in = feat
    ...
    def forward_multi_level(self, query_res_list, global_nerf_feature=None, reduce_opt='mean'): 
        aux = {}
        if len(query_res_list) > 0:
            sampled_color, sampled_Rw2c, sampled_dir, sampled_conf, sampled_embedding, sampled_xyz_pers, sampled_xyz, \
                sample_pnt_mask, sample_loc, sample_loc_w, sample_loc_mask_tensor, sample_ray_dirs, ray_mask_tensor, raypos_mask_tensor, vsize, \
                    grid_vox_sz, raypos_tensor = query_res_list[0]         

        if len(query_res_list) == 1 and self.opt.backbone_opt == "none":
            # point-nerf's baseline.
            # single level
            decoded_features, ray_valid, weight, conf_coefficient = self.forward(
                sampled_color, sampled_Rw2c, sampled_dir, sampled_conf, sampled_embedding, 
                sampled_xyz_pers, sampled_xyz, sample_pnt_mask, sample_loc, 
                sample_loc_w, sample_ray_dirs, vsize, grid_vox_sz) 

            aux["sample_loc"] = sample_loc 
            aux["vsize"] = vsize
            aux["ray_mask_tensor"] = ray_mask_tensor  
            aux["weight"] = weight 
            aux["conf_coefficient"] = conf_coefficient 
            return decoded_features, ray_valid, aux
        ...
        if len(query_res_list) > 0:
            ...
            # raypos_mask_tensor.shape: BxnxK
            # ray_mask_tensor.shape: BxN 
            b, N = ray_mask_tensor.shape[:2]
            R = raypos_mask_tensor.shape[2] 
            device = ray_mask_tensor.device
            raypos_mask_tensor.shape[2]
        else:          
            raypos_tensor = global_nerf_feature["raypos_tensor"]
            b, N, R = raypos_tensor.shape[:3]
            vsize = [1.0] * 3 # no constraint. 
            device = raypos_tensor.device

        segment_idx_full = torch.arange(
            b*N*R, dtype=torch.long, device=device, 
            ).reshape([b, N, R])
        ...
        ray_mask_tensor_list = [] 
        weights_list = []
        conf_coefficient_list = [] 

        render_sub_idx = getattr(self.opt, 'render_sub_idx', []) 
        idx_layer_render_list = [] 
        
        for idx_layer, query_res in enumerate(query_res_list):
            sampled_color, sampled_Rw2c, sampled_dir, sampled_conf, sampled_embedding, sampled_xyz_pers, sampled_xyz, \
                sample_pnt_mask, sample_loc, sample_loc_w, sample_loc_mask_tensor, sample_ray_dirs, ray_mask_tensor, raypos_mask_tensor, vsize, \
                    grid_vox_sz, raypos_tensor = query_res  
    
            # Torch scatter.

            segment_idx = segment_idx_full[ray_mask_tensor>0]
            if segment_idx.numel() == 0:
                continue
            if len(render_sub_idx) > 0:
                if idx_layer not in render_sub_idx:
                    continue
            idx_layer_render_list += [idx_layer]
            segment_idx = segment_idx.reshape(*raypos_mask_tensor.shape)
            segment_idx = segment_idx[raypos_mask_tensor>=0] # follow the pointnerf's setting
            segment_idx_list += [segment_idx]

            # why do we need this weight. 
            decoded_features, viewdirs, ray_valid, weight, conf_coefficient = self.forward(
                sampled_color, sampled_Rw2c, sampled_dir, sampled_conf, sampled_embedding, 
                sampled_xyz_pers, sampled_xyz, sample_pnt_mask, sample_loc, 
                sample_loc_w, sample_ray_dirs, vsize, grid_vox_sz, idx_layer=idx_layer)
            sample_loc_mask = sample_loc_mask_tensor>0
            features_list += [decoded_features[sample_loc_mask]]
            viewdirs_list += [viewdirs[sample_loc_mask]]
            ray_valid_list += [ray_valid[sample_loc_mask]]
            sample_loc_list += [sample_loc[sample_loc_mask]]

            if weight is not None:
                weights_list += [weight[sample_loc_mask].reshape(-1)]
            if conf_coefficient is not None:
                conf_coefficient_list += [conf_coefficient[sample_loc_mask].reshape(-1)]
            ray_mask_tensor_list += [ray_mask_tensor]

        global_render = ((-1 in render_sub_idx) or (self.opt.num_point_emb_level+1 in render_sub_idx)) \
            if len(render_sub_idx) > 0 else True
        if global_nerf_feature is not None and global_render: 
            idx_layer_render_list += [-1]
            # all ray
            # global_nerf_feature = {
            #     "decoded_features": global_nerf_decoded_features, 
            #     "raydir": raydir,
            #     "sample_loc":  raypos_tensor,
            # }
            decoded_features_ = global_nerf_feature['decoded_features']
            sample_loc_ = global_nerf_feature["sample_loc"]
            viewdirs_ = positional_encoding(global_nerf_feature['raydir'], freqs=self.num_viewdir_freqs, ori=True)[..., 3:]
            segment_idx_ = segment_idx_full.reshape(-1)

            features_list += [decoded_features_.reshape(-1, decoded_features_.shape[-1])] 
            sample_loc_list += [sample_loc_.reshape(-1, sample_loc_.shape[-1])]
            viewdirs_list += [viewdirs_[:, :, None].expand(-1, -1, R, -1).reshape(-1, viewdirs_.shape[-1])]
            segment_idx_list += [segment_idx_]
            ray_valid_list += [torch.ones((b, N, R), dtype=torch.float, device=device).reshape(-1) > 0]  
            ray_mask_tensor_list += [torch.ones((b, N), dtype=torch.float, device=device) > 0]
            # we don't have weights and co-efficient here and so ignore. 
            
        
        # Aggregation between levels.
        if len(segment_idx_list) == 0:
            # nothing queried at all 
            # torch.zeros(in_shape[:-1] + (self.opt.shading_color_channel_num + 1,), device=ray_valid.device, dtype=torch.float32), ray_valid.view(in_shape[:-1]), None, None
            # make some default behavior.
            output_feature_holder = torch.zeros((b, 0, R, self.opt.shading_color_channel_num + 1), device=ray_mask_tensor.device, dtype=torch.float32) 
            output_ray_valid_holder = torch.zeros((b, 0, R), device=ray_mask_tensor.device, dtype=torch.float32)
            output_sample_loc_holder = torch.zeros((b, 0, R, 3), device=ray_mask_tensor.device, dtype=torch.float32) 
            aux["sample_loc"] = output_sample_loc_holder  
            aux["vsize"] = vsize
            aux["weight"] = None 
            aux["conf_coefficient"] = None 
            aux["ray_mask_tensor"] = ray_mask_tensor 
            return output_feature_holder, output_ray_valid_holder, aux

        # if len(render_sub_idx) > 0: 
        #     def index_list(list, list_idx):
        #         return [list[idx] for idx in list_idx]
        #     segment_idx_list = index_list(segment_idx_list, render_sub_idx)
        #     features_list = index_list(features_list, render_sub_idx)
        #     viewdirs_list = index_list(viewdirs_list, render_sub_idx)
        #     ray_valid_list = index_list(ray_valid_list, render_sub_idx)
        #     sample_loc_list = index_list(sample_loc_list, render_sub_idx)
        #     ray_mask_tensor_list = index_list(ray_mask_tensor_list, render_sub_idx)
        
                    
        segment_idx_all = torch.cat(segment_idx_list, 0)

        feature_all = torch.cat(features_list, 0)
        viewdirs_all = torch.cat(viewdirs_list, 0)
        ray_valid_all = torch.cat(ray_valid_list, 0)
        sample_loc_all = torch.cat(sample_loc_list, 0) 
        ray_mask_tensor_all_reduce = torch.any(torch.stack(ray_mask_tensor_list, dim=0), dim=0)

        weights_all = torch.cat(weights_list) if len(weights_list) > 0 else None
        conf_coefficient_all = torch.cat(conf_coefficient_list) if len(conf_coefficient_list) > 0 else None

        # with torch.no_grad():
        unique_idx, reverse_idx = segment_idx_all.unique(return_inverse=True)
    
        feature_all_reduce = torch_scatter.scatter(feature_all, reverse_idx, dim=0, reduce=reduce_opt)
        viewdirs_all_reduce = torch_scatter.scatter(viewdirs_all, reverse_idx, dim=0, reduce=reduce_opt) 
        # sample_loc_all_reduce_std = torch_scatter.scatter_std(sample_loc_all, reverse_idx, dim=0) # double check
        sample_loc_all_reduce = torch_scatter.scatter(sample_loc_all, reverse_idx, dim=0, reduce=reduce_opt) 
        ray_valid_all_reduce = torch_scatter.scatter(ray_valid_all.int(), reverse_idx, dim=0, reduce=reduce_opt) > 0

        decoded_features = self.get_radiance(feature_all_reduce, viewdirs_all_reduce)  

        # ray_valid
        output_ray_valid_holder = torch.zeros([b*N*R], dtype=torch.bool, device=device) 
        output_ray_valid_holder[unique_idx] = ray_valid_all_reduce
        output_ray_valid_holder = output_ray_valid_holder.reshape([b, N, R])
        output_ray_valid_holder = output_ray_valid_holder[ray_mask_tensor_all_reduce].reshape([b, -1, R])

        # feature
        output_feature_holder = torch.zeros([b*N*R, self.opt.shading_color_channel_num + 1], dtype=torch.float32, device=device) 
        output_feature_holder[unique_idx] = decoded_features
        output_feature_holder = output_feature_holder.reshape([b, N, R, self.opt.shading_color_channel_num + 1])
        output_feature_holder = output_feature_holder[ray_mask_tensor_all_reduce].reshape([b, -1, R, self.opt.shading_color_channel_num + 1])

        # recover the init sample locs
        output_sample_loc_holder = torch.zeros([b*N*R, sample_loc_all_reduce.shape[-1]], dtype=torch.float32, device=device) 
        output_sample_loc_holder[unique_idx] = sample_loc_all_reduce 
        output_sample_loc_holder = output_sample_loc_holder.reshape([b, N, R, sample_loc_all_reduce.shape[-1]])
        output_sample_loc_holder = output_sample_loc_holder[ray_mask_tensor_all_reduce].reshape([b, -1, R, sample_loc_all_reduce.shape[-1]])

        aux["sample_loc"] = output_sample_loc_holder
        aux["vsize"] = vsize
        aux["weight"] = weights_all 
        aux["conf_coefficient"] = conf_coefficient_all 
        aux["ray_mask_tensor"] = ray_mask_tensor_all_reduce 
        
        return output_feature_holder, output_ray_valid_holder, aux
    
        
    def viewmlp_multilevel(self, sampled_color, sampled_Rw2c, sampled_dir, sampled_conf, sampled_embedding, sampled_xyz_pers, sampled_xyz, sample_pnt_mask, sample_loc, sample_loc_w, sample_ray_dirs, vsize, weight, pnt_mask_flat, pts, viewdirs, total_len, ray_valid, in_shape, dists, idx_layer=0):
        # print("sampled_Rw2c", sampled_Rw2c.shape, sampled_xyz.shape)
        # assert sampled_Rw2c.dim() == 2
        B, R, SR, K, _ = dists.shape
        sampled_Rw2c = sampled_Rw2c.transpose(-1, -2)
        ...
        # viewdirs = viewdirs[ray_valid, :]
        is_reduce_size = self.is_reduce_size_list[idx_layer]
        ...
        if self.opt.agg_intrp_order == 0:
            ...
            if self.opt.num_feat_freqs > 0 and not is_reduce_size:
                feat = torch.cat([feat, positional_encoding(feat, self.opt.num_feat_freqs)], dim=-1)
            feat = torch.cat([feat, dists_flat], dim=-1)
            weight = weight.view(B * R * SR, K, 1)
            pts = pts_pnt
        ...
        if self.opt.agg_feat_xyz_mode != "None":
            feat = torch.cat([feat, pts], dim=-1)
        # print("feat",feat.shape) # 501
        block1 = getattr(self, f'block1_{idx_layer}')
        feat = block1(feat)
        ...
        if self.opt.shading_feature_mlp_layer2>0:
            ...
            if self.opt.agg_intrp_order > 0:
                feat = torch.cat([feat, dists_flat], dim=-1)
            block2 = getattr(self, f'block2_{idx_layer}')
            feat = block2(feat)
            # feat = self.block2(feat)
        ...
        if self.opt.shading_feature_mlp_layer3>0:
            ...
            if sampled_dir is not None:
                ...
                if self.opt.apply_pnt_mask > 0:
                    ori_viewdirs = ori_viewdirs[pnt_mask_flat, :]
                feat = torch.cat([feat, sampled_dir - ori_viewdirs, torch.sum(sampled_dir*ori_viewdirs, dim=-1, keepdim=True)], dim=-1)
            block3 = getattr(self, f'block3_{idx_layer}')
            feat = block3(feat)
        ...
        output = feat  
        output_placeholder = torch.zeros([total_len, feat.shape[-1]], dtype=torch.float32, device=output.device)
        output_placeholder[ray_valid] = output
        return output_placeholder, viewdirs  