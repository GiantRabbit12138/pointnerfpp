...
class MvsPointsVolumetricModel(NeuralPointsVolumetricModel):
    ...
    def setup_optimizer(self, opt):
        ...
        for name in self.model_names:
            ...
            if name == "mvs":
                ...
                net_params = net_params + [par[1] for par in param_lst if not par[0].startswith("module.neural_points") and not par[0].startswith("module.proposal_networks")]
                neural_params = neural_params + [par[1] for par in param_lst if par[0].startswith("module.neural_points")]

        print("all net_params", [(par[0], par[1].shape, par[1].requires_grad)  for par in param_lst if not par[0].startswith("module.neural_points")])
        print("except proposal net_params", [(par[0], par[1].shape, par[1].requires_grad)  \
                                             for par in param_lst if not par[0].startswith("module.neural_points") and not par[0].startswith("module.proposal_networks")])
    ...
    def load_networks(self, epoch, strict=False):
