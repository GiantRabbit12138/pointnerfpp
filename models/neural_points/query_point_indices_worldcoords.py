...
class lighting_fast_querier():
    ...
    def query_points(self, pixel_idx_tensor, point_xyz_pers_tensor, point_xyz_w_tensor, actual_numpoints_tensor, h, w, intrinsic, near_depth, far_depth, ray_dirs_tensor, cam_pos_tensor, cam_rot_tensor, raypos_tensor=None):
        ...
        return sample_pidx_tensor, self.w2pers(sample_loc_w_tensor, cam_rot_tensor, cam_pos_tensor), sample_loc_w_tensor, sample_loc_mask_tensor, sample_ray_dirs_tensor, ray_mask_tensor, raypos_mask_tensor, vsize_np, ranges_np, raypos_tensor
    ...
    def query_grid_point_index(self, h, w, pixel_idx_tensor, raypos_tensor, point_xyz_w_tensor, actual_numpoints_tensor, kernel_size_gpu, query_size_gpu, SR, K, ranges_np, scaled_vsize_np, scaled_vdim_np, vscale_np, max_o, P, radius_limit_np, depth_limit_np, ranges_gpu, scaled_vsize_gpu, scaled_vdim_gpu, vscale_gpu, ray_dirs_tensor, cam_pos_tensor, kMaxThreadsPerBlock = 1024):
        ...
        if R > 0:
            ...
            # assert torch.all(sample_loc_tensor[sample_loc_mask_tensor>0] == raypos_tensor[raypos_mask_tensor>=0]) 
            ...
            # TODO: why valid points has neighbhoring points assigned? 
            ...
            sample_loc_mask_tensor = torch.masked_select(sample_loc_mask_tensor, masked_valid_ray[..., None].expand(-1, -1, SR)).reshape(B, R, SR) 
            raypos_mask_tensor = torch.masked_select(raypos_mask_tensor, masked_valid_ray[..., None].expand(-1, -1, D)).reshape(B, R, D) 
        # self.count+=1
        return sample_pidx_tensor, sample_loc_tensor, sample_loc_mask_tensor, ray_mask_tensor.to(torch.int8), raypos_mask_tensor
