...
std::vector<torch::Tensor> woord_query_grid_point_index_cuda(
  ...
  if(R > 0){
        ...
        sample_loc_mask_tensor = torch::masked_select(sample_loc_mask_tensor, masked_valid_ray.unsqueeze(-1).expand({-1, -1, SR})).reshape({B, R, SR});
        raypos_mask_tensor = torch::masked_select(raypos_mask_tensor, masked_valid_ray.unsqueeze(-1).expand({-1, -1, D})).reshape({B, R, D});
  }

  return {sample_pidx_tensor, sample_loc_tensor, sample_loc_mask_tensor, ray_mask_tensor.toType(torch::kInt8), raypos_mask_tensor};
}