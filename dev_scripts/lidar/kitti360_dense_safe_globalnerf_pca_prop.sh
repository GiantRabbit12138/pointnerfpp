...
# bash dev_scripts/lidar/kitti360_dense_safe_globalnerf_pca.sh kitti360/abs/no_contract identity mvs 2.92 learned 3 train_00 0 grid_sampling 0.08 pts_planes 2 constant_ones -2 1 trainval triplane
moremode="trainval"
if [ "$split" == "$moremode" ]; then
    echo "trainval mode: more iterations."
    maximum_step=200000 
else
    echo "train mode: less iterations."
    maximum_step=100000 
fi
...
min_k=2
global_nerf_t=pca
echo "Mink neighborhood size $min_k"
...
if [ $resample_pnts -eq 0 ]; then
    nrCheckpoint="../logs/logs_cur/${suffix}/prop_${prop_opt}_${global_nerf}_${agg_opt}_pyramid_${pyramid_opt}_backbone_${backbone_opt}_conf_${mid_conf_mode}_${point_conf_mode}_point_emb_opt_${point_emb_opt}_vs_${backbone_voxel_size}_init_${point_init_emb_opt}_emb_level_${num_point_emb_level}_prob_freq_${prob_freq}_planesize_${plane_size}__splane_${start_plane_idx}_${split}"
else
    nrCheckpoint="../logs/logs_cur/${suffix}/npts${resample_pnts}_prop_${prop_opt}_${global_nerf}_${agg_opt}_pyramid_${pyramid_opt}_backbone_${backbone_opt}_conf_${mid_conf_mode}_${point_conf_mode}_point_emb_opt_${point_emb_opt}_vs_${backbone_voxel_size}_init_${point_init_emb_opt}_emb_level_${num_point_emb_level}_prob_freq_${prob_freq}_planesize_${plane_size}__splane_${start_plane_idx}_${split}"
fi
...
if [ $prob_freq -eq 0 ]; then
    ...
    total_step=$((($maximum_step - 0) / $prob_freq)) 
    echo "PR strategy"
fi
