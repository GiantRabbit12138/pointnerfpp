...
def test(model, dataset, visualizer, opt, bg_info, test_steps=0, gen_vid=False, lpips=True):
    print('-----------------------------------Testing-----------------------------------')
    model.eval()
    model.net_ray_marching.module.return_depth = True
    ...
    for i in range(0, total_num, opt.test_num_step): # 1 if test_steps == 10000 else opt.test_num_step 
        ...
        # visualizer.save_pts_2d(data, i, opt=opt)
        if visualizer.check_if_rendered(["gt_image", "coarse_raycolor"], i, opt=opt):
            continue
        raydir = data['raydir'].clone()
        pixel_idx = data['pixel_idx'].view(data['pixel_idx'].shape[0], -1, data['pixel_idx'].shape[3]).clone()
        ...
        for k in range(0, totalpixel, chunk_size):
            ...
            if visuals is None:
                ...
                for key, value in curr_visuals.items():
                    ...
                    if key == 'coarse_depth':
                        chunk = value.cpu().numpy()
                        visuals[key] = np.zeros((height, width, 1)).astype(chunk.dtype)
        ...
        for key, value in visuals.items():
            ...
            if key == 'coarse_depth':
                visualizer.print_details("{}:{}".format(key, visuals[key].shape))
                visuals[key] = visuals[key].reshape(height, width)
...
def main():
    ...
    opt.show_tensorboard = True

    if opt.normalize_ray > 0:
        opt.dir_norm = 1
    ...
    if opt.dataset_name in ['kitti360']:
        opt.ranges = None# hard to predefined # used to range from lidar. 
    
    # loading pointnerf's points. We encoding the hps from pointnerf here 
    # so that we can a single bash file to launch jobs.
    if opt.dataset_name in ['nerf_synth360_ft']:
        ranges_dict = {
            'chair':    [-0.721, -0.695, -0.995, 0.658, 0.706, 1.050],
            'drums':    [-1.126, -0.746, -0.492, 1.122, 0.962, 0.939],
            'ficus':    [-0.377, -0.858, -1.034, 0.555, 0.578, 1.141],
            'hotdog':   [-1.198, -1.286, -0.190, 1.198, 1.110, 0.312],
            'lego':     [-0.638, -1.141, -0.346, 0.634, 1.149, 1.141],
            'materials': [-1.123, -0.759, -0.232, 1.072, 0.986, 0.200],
            'mic':      [-1.252, -0.910, -0.742, 0.767, 1.082, 1.151],
            'ship':     [-1.277, -1.300, -0.550, 1.371, 1.349, 0.729],
        }
        opt.ranges = ranges_dict[opt.scan]     
        print(f"{opt.scan}, {opt.ranges}")
    ...
    with torch.no_grad():
        ...
        if len([n for n in glob.glob(opt.checkpoints_dir + opt.name + "/*_net_ray_marching.pth") if os.path.isfile(n)]) > 0:
            ...
            opt.mode = 2
            model = create_model(opt)
        elif opt.load_points == 8: 
            # cleaner
            load_points = opt.load_points 
            opt.load_points = 0
            
            points_xyz_all, points_embedding_all, points_conf_all, points_color_all, points_dir_all = \
                train_dataset.load_points_field() 
            ...
            model.set_points(points_xyz_all, points_embedding_all, points_color=points_color_all,
                             points_dir=points_dir_all, points_conf=points_conf_all,
            ...
            if opt.ranges[0] > -99.0 or opt.ranges is not None:
            ...
            if opt.resample_pnts > 0:
                ...
                dir_save = os.path.join(opt.checkpoints_dir, opt.name, 'pts_vis_dump') 
            ...
            if opt.pts0_init == "random":
                uni_size = np.max(np.abs(np.array(opt.ranges)))
                print(points_xyz_all.min(dim=0)[0], points_xyz_all.max(dim=0)[0])
                points_xyz_all = (torch.rand_like(points_xyz_all) * 2  -1) * uni_size 
                dir_save = os.path.join(opt.checkpoints_dir, opt.name, 'pts_vis_dump') 
                pts_utils.export_pointcloud(f'{dir_save}/after_random_{len(points_xyz_all)}.ply', points_xyz_all)
                print(points_xyz_all.min(dim=0)[0], points_xyz_all.max(dim=0)[0])
        ...
        if points_xyz_all is not None and load_points not in [8]:
            if opt.bgmodel.startswith("planepoints"):
            ...
            point_emb_opt = getattr(opt, 'point_emb_opt', 'mvs') 
            if point_emb_opt == 'uniform': 
                points_color_all.uniform_(0, 1)
                points_embedding_all.uniform_(-1,1)
    ...
    test_opt.show_tensorboard = False 
    test_opt.is_train = False
    test_opt.random_sample = 'no_crop'
    test_opt.random_sample_size = min(24, opt.random_sample_size)
    ...
    if False:
        # testing 
        # test_opt.split = "trainval" 
        for i in range(opt.num_point_emb_level +2):
            model.aggregator.opt.render_sub_idx = [i_ for i_ in range(i) ] + [-1] 
            # model.aggregator.opt.render_sub_idx = [i] 
            # test_opt.split = "test_benchmark" if opt.dataset_name in ['kitti360'] else test_opt.split 
            test_opt.split = "render" 
            test_opt.name = opt.name + f"/{test_opt.split}_{total_steps}_{model.aggregator.opt.render_sub_idx}_983_998"
            ...
            test_dataset[0]
            with torch.no_grad():
                test_psnr = test(model, test_dataset, Visualizer(test_opt), test_opt, test_bg_info, test_steps=total_steps, gen_vid=False, lpips=True)
    ...
    if False:
        ...
        test_opt.split = "render"
        # test_opt.name = opt.name + f"/{test_opt.split}_{total_steps}_allframes_pcd"
        test_opt.name = opt.name + f"/{test_opt.split}_{total_steps}_denseframe_spline_large"
        test_opt.test_num_step = 1 
        test_dataset = create_dataset(test_opt) 
        test_dataset[0]
        # test_dataset = create_test_dataset(test_opt, opt, total_steps, test_num_step=1)
    ...
    if total_steps >= opt.maximum_step:
        # testing 
        # test_opt.split = "trainval" 
        test_opt.split = "test_benchmark" if opt.dataset_name in ['kitti360'] else test_opt.split 
        test_opt.name = opt.name + f"/{test_opt.split}_{total_steps}_time_comprison"
        test_opt.test_num_step = 1 
        test_dataset = create_dataset(test_opt) 
        # test_dataset = create_test_dataset(test_opt, opt, total_steps, test_num_step=1)
        model.opt.no_loss = 1
    ...
    ##### testing on test frames.
    if opt.dataset_name in ['kitti360']:
        test_opt.split = "test_benchmark"
        test_opt.name = opt.name + f"/{test_opt.split}_{total_steps}"
        test_opt.test_num_step = 1 
        test_dataset = create_dataset(test_opt) 
        # test_dataset = create_test_dataset(test_opt, opt, total_steps, test_num_step=1)
        model.opt.no_loss = 1
