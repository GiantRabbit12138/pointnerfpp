...
try:
    ...
    def compare_ssim(gt, img, win_size, multichannel=True, channel_axis=2, data_range=1.0):
        return structural_similarity(
            gt, img, win_size=win_size, 
            channel_axis=channel_axis, multichannel=multichannel, data_range=data_range)
...
def report_metrics(gtFolder, imgFolder, outFolder, metrics, id_list, imgStr="step-%04d-fine_raycolor.png", gtStr="step-%04d-gt_image.png", use_gpu=True, print_info=True):
    total ={}
    ...
    for i in id_list:
        ...
        for key in metrics:
            if key == "psnr":
                ...
                val = compare_ssim(gt, img, 11, multichannel=True, channel_axis=2, data_range=1.0)
            elif key == "lpips":
